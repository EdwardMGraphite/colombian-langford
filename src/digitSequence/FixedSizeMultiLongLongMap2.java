 ...
 */
public class FixedSizeMultiLongLongMap2 {
	...
	public FixedSizeMultiLongLongMap2(int hashLength, int storage, int keySize) {
	...
	public synchronized long get(long[] key){
		...
		while (pointer != 0){
			...
			boolean found = true;
			for (int i = 0; i < keySize; i++) {
				if (key[i] != array[i][pointer]) {
					found = false;
					break;
				}
			}
			if (found)
				return array[keySize][pointer];
			
			boolean trialKeyGreater = false;
			
			for (int i = 0; i < keySize; i++) {
				if (array[i][pointer] > key[i]) {
					trialKeyGreater = true;
					break;
				}
				else if (array[i][pointer] < key[i])
					break;
			}
			
			if (trialKeyGreater)
				pointer = (int) (array[keySize + 1][pointer] >>> 32);
			else
				pointer = (int) (array[keySize + 1][pointer] & BITMASK);
	...
	public synchronized void put(long[] key, long value){
		...
		if (pointer == 0){
			...
			for (int i = 0; i < keySize; i++)
				array[i][pointer] = key[i];
			array[keySize][pointer] = value;
			return;
		}
		...
		for (int i = 0; i < keySize; i++) {
			if (key[i] > array[i][oldPointer]) {
				keyGreater = true;
				break;
			}
			else if (key[i] < array[i][oldPointer] )
				break;
		}
		
		if (keyGreater)
			array[keySize + 1][oldPointer] |= pointer;
		else
			array[keySize + 1][oldPointer] |= ((long) pointer << 32);
	}
